...
template <class T0> ALWAYS_INLINE uint64_t syscall_stub(uint64_t code, T0 _0)
{
    register auto syscall_id asm("a7") = code;
...
template <class T0, class T1> ALWAYS_INLINE uint64_t syscall_stub(uint64_t code, T0 _0, T1 _1)
{
    register auto syscall_id asm("a7") = code;
    register auto _a0 asm("a0")        = _0;
